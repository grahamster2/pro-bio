...
import { CheckCircle, XCircle, ExternalLink, ShieldAlert, AlertTriangle } from "lucide-react";
...
export default async function AdminDashboard() {
                ...
                <div className="max-w-2xl mx-auto">
                    <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-6">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <h1 className="text-2xl font-bold text-red-500">Admin Not Configured</h1>
                        </div>
                        <p className="text-slate-300 mb-4">
                            The ADMIN_USER_ID environment variable is not set. Please configure it to access the admin dashboard.
                        </p>
                        <Link 
                            href="/dashboard" 
                            className="inline-flex items-center gap-2 text-brand-amber hover:text-amber-400 transition-colors"
                        >
                            ← Back to Dashboard
                        </Link>
                    </div>
    ...
    if (!userId || userId !== process.env.ADMIN_USER_ID) {
        redirect('/dashboard'); // Redirect to dashboard instead of home
    }
    ...
    try {
        ...
        const ApproveForm = ({ profileId }: { profileId: string }) => {
            return (
                <form action={async () => {
                    'use server'
                    await updateVerificationStatus(profileId, 'Verified')
                }}>
                    <button type="submit" className="w-full flex items-center justify-center gap-2 bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 hover:bg-emerald-500 hover:text-zinc-950 px-6 py-2.5 rounded-lg font-bold transition-all text-sm">
        ...
        const RejectForm = ({ profileId }: { profileId: string }) => {
            return (
                <form action={async () => {
                    'use server'
                    await updateVerificationStatus(profileId, 'Rejected')
                }}>
                    <button type="submit" className="w-full flex items-center justify-center gap-2 bg-red-500/10 text-red-500 border border-red-500/20 hover:bg-red-500 hover:text-white px-6 py-2.5 rounded-lg font-bold transition-all text-sm">
        ...
        const ProfileCard = ({ profile, type }: { profile: any, type: 'pending' | 'verified' | 'rejected' }) => (
            <div key={profile.id} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-xl flex flex-col md:flex-row gap-6 items-start md:items-center justify-between mb-4">
                ...
                <div className="flex flex-row md:flex-col gap-3 w-full md:w-auto shrink-0 border-t md:border-t-0 md:border-l border-zinc-800 pt-4 md:pt-0 md:pl-6">
                    {type !== 'verified' && <ApproveForm profileId={profile.user_id} />}
                    {type !== 'rejected' && <RejectForm profileId={profile.user_id} />}
                </div>
            </div>
        );
            ...
            <div className="min-h-screen bg-zinc-950 text-slate-100 p-8">
                <div className="max-w-2xl mx-auto">
                    <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-6">
                        <div className="flex items-center gap-3 mb-4">
                            <AlertTriangle className="w-6 h-6 text-red-500" />
                            <h1 className="text-2xl font-bold text-red-500">Admin Dashboard Error</h1>
                        </div>
                        <p className="text-slate-300 mb-4">
                            There was an error loading the admin dashboard. Please check the server logs and try again.
                        </p>
                        <Link 
                            href="/dashboard" 
                            className="inline-flex items-center gap-2 text-brand-amber hover:text-amber-400 transition-colors"
