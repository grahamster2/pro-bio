...
export default function DashboardClient({ initialProfile }: { initialProfile: any }) {
    ...
    return (
        <div className="grid lg:grid-cols-2 gap-4 md:gap-8 flex-1 min-h-0 pb-24 md:pb-10">
            ...
            <div className="flex flex-col bg-zinc-900/40 backdrop-blur-xl border border-zinc-800/80 rounded-2xl md:rounded-3xl overflow-hidden md:h-full shadow-2xl relative">
            ...
            <div id="tutorial-preview" className="hidden lg:flex items-center justify-center bg-zinc-950/30 border border-zinc-800 rounded-2xl p-6 overflow-hidden relative group">
            ...
            <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden">
                <div className="bg-zinc-950/95 backdrop-blur-xl border-t border-zinc-800/80 px-4 py-3 flex items-center gap-3 safe-area-pb">
                    {message && (
                        <span className="text-xs font-medium text-brand-amber flex-1 truncate">{message}</span>
                    )}
                    <button
                        onClick={handleSave}
                        disabled={saving}
                        className="bg-brand-amber text-zinc-950 px-6 py-2.5 rounded-xl font-black text-sm hover:bg-amber-400 transition-all shadow-[0_0_15px_rgba(245,158,11,0.15)] flex items-center gap-2 disabled:opacity-50 ml-auto"
                    >
